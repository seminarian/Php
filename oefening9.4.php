...
if($_GET["action"] == "bewerk") {
	$filmke = New film($_GET["id"],$_POST["titel"],$_POST["duurtijd"]);
	$mlijst->updateFilm($filmke);
}
...
<body>
	...
	foreach ($filmlijst as $film) {
		print("<li>".$film->getTitel()." (".$film->getDuurtijd().") <a href=\"oefening9.4.php?action=verwijder&id=".$film->getId()."\">Verwijderen</a>.<a href=\"oefening9.4.bewerk.php?action=bewerk&id=".$film->getId()."\">Bewerken</a></li>");
	}
	?>
	</ul>

	<h1>Film toevoegen</h1>
	<form action="oefening9.4.php" method="POST">
